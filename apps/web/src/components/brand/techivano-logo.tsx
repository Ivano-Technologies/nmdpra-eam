...
function Mark({
  ...
  const markMotion = motion === "hover-tilt" ? "hover-tilt" : motion === "spin-slow" ? "spin-slow" : "none";
  return (
    <TechivanoMark
      size={s.mark}
...
/**
 * Horizontal Techivano branding: mark + wordmark (+ optional tagline).
 */
export function TechivanoLogo({
  className,
  markClassName,
  size = "md",
  showTagline = false,
  ...
  const s = sizes[size];
  const label = wordmark === "product" ? PRODUCT_NAME : BRAND_NAME;
  const markMotion: "none" | "hover-tilt" | "spin-slow" =
        ...
        <span className={cn("inline-flex items-center", className)}>
          <Tooltip>
            ...
            <TooltipContent side="bottom">
              ...
              <p className="text-muted-foreground max-w-[240px] text-[11px]">{BRAND_NAME}</p>
            </TooltipContent>
          </Tooltip>
        </span>
      );
